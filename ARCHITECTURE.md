...
이 프로젝트에는 4개의 로컬 엔진이 있고, **색인(Index)과 검색(Search)
에서 관여하는 정도가 전부 다르다.**

| 엔진 | 종류 | 색인 능력 | 검색 능력 |
|---|---|---|---|
| **Ollama** | 임베딩 엔진 | ✅ 청크 → 벡터 변환(저장할 벡터 생성) | ✅ 질의 → 벡터 변환(비교할 벡터 생성) |
| **Chroma** | 벡터 저장소 | ✅ 벡터 저장/관리(HNSW 인덱스 구축) | ✅ 코사인 유사도 검색(HNSW 근접 탐색) |
| **BM25 (rank_bm25)** | 키워드 검색 엔진 | ❌ 없음(자체 저장소 없음, 검색 시점에 즉석 계산) | ✅ 키워드 매칭(TF-IDF 계열 점수) |
| **LangGraph** | 오케스트레이션 엔진 | ❌ 없음 | ✅ 모드 분기 + RRF 융합(검색 방식 제어) |

### 핵심 차이

1. **"색인 능력"이 있는 건 Ollama + Chroma 둘뿐** — 색인 파이프라인은
   `문서 → 청크 분할 → Ollama(벡터화) → Chroma(저장)` 이다. BM25 와
   LangGraph 는 색인 단계에 관여하지 않는다.
2. **"검색 능력"은 세 엔진이 각기 다른 방식으로 개입** — vector/hybrid 는
   Ollama(질의 벡터화) + Chroma(코사인), keyword/hybrid 는 BM25(키워드
   매칭), hybrid 는 LangGraph 가 두 결과를 RRF 로 융합한다.
3. **BM25 는 저장소가 없다** — 별도 인덱스를 미리 만들어 두지 않고,
   검색 시점에 모든 청크 텍스트를 읽어(`fetch_all_documents`) 즉석에서
   Okapi BM25 점수를 계산한다. 그래서 색인 능력은 없고 keyword 검색만
   전담한다.
...
[청크 분할]                         │
 │                                 │
 ▼                                 │
Ollama ──(벡터)──► Chroma  ◄──(질의 벡터)── Ollama
 (만들기)          (저장)          │
                    │              │  (vector/hybrid)
                    ▼              │
               코사인 유사도 검색 ──┐
                                    │  → (hybrid) LangGraph: RRF 융합
               BM25 키워드 검색 ─────┘    (keyword/hybrid)
                 (검색 시점에 즉석 계산, 색인 없음)
```

**요약**: Ollama 와 Chroma 는 "색인(만들고 저장) + 검색(비교)" 양쪽에
