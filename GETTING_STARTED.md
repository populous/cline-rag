...
### OpenCode 창을 열어서 실제로 확인하기

등록이 끝났으면, 이 프로젝트 폴더에서 OpenCode 를 실행합니다.
...
터미널 전체가 채팅 UI(TUI)로 바뀝니다. 아래를 확인하세요.

1. **MCP 연결 확인**: 화면 하단이나 MCP 서버 목록에 `cline-rag` 가 연결된
   상태로 보이면 정상입니다.
2. **자연어로 바로 질문**: 별도 명령어 없이 그냥 채팅창에 입력하면 됩니다.
   ```
   연차는 반차 단위로도 신청할 수 있어?
   ```
   `AGENTS.md` 의 규칙("답변 전에 검색한다")에 따라 OpenCode 가 **스스로**
   `search_docs` 도구를 호출하고, 그 검색 결과(문서 발췌 + 파일 경로)를
   근거로 답합니다 — 사람이 `search_docs` 를 직접 부를 필요가 없습니다.
3. **정상 동작의 특징**: 답변 앞뒤로 도구 호출(tool call) 표시가 나오고,
   답변 끝에 참고한 파일 경로(예: `docs\example_vacation_policy.md`)가
   함께 언급됩니다. 문서에 없는 내용은 "모른다"고 답하는 것도 정상입니다
   (`AGENTS.md` 가 지어내는 것을 금지하기 때문).

> 이 확인 과정은 3단계의 `ask.ps1` 결과와 동일한 검색 로직
> (`rag_core.search_documents()`)을 씁니다. 즉 `ask.ps1` 로 이미 확인한
> 결과가 잘 나왔다면, OpenCode 에서도 동일하게 동작해야 정상입니다.
